...
Rock* Game::createRock(int whichRock, Position pInit, Velocity vInit, bool isInitial) {
    ...
    if (Trig::getMagnitude(pInit.getX(), pInit.getY()) < 10) {
        ...
        if (isInitial) {
            ...
            if (ship != NULL) { // if ship is null (error), then skip
                ...
                while (dist(shipPosition, pInit) < buffer) { // redo random point
...
std::pair<Velocity, Velocity> generateOffsetVelocity(MovingObject rock, MovingObject obj) {
    ...
    // - offset speeds depend on size of rock (get info from rocks.h based on rockID)
    ...
    double randomAngleOffset = Trig::rad(random(0, 15)); // = ø
...
void Game::splitRock(Rock & rock, MovingObject & obj) {
    ...
    newRock1 = createRock(
        newRockIndex, rock.getPosition(), rock.getVelocity() + offsetV1, false);
    newRock2 = createRock(
        newRockIndex, rock.getPosition(), rock.getVelocity() + offsetV2, false);
...
bool Game::checkCollision(MovingObject* obj1, MovingObject* obj2) {
    ...
    double tooClose = obj1->getRadius() + obj2->getRadius();
...
double Game::getClosestDistance(const MovingObject &obj1, const MovingObject &obj2) {
    // find the maximum distance traveled
    double dMax = std::max(abs(obj1.getVelocity().getDx()), abs(obj1.getVelocity().getDy()));
    ...
    dMax = std::max((float)dMax, 0.1f); // when dx and dy are 0.0. Go through the loop once.

    double distMin = std::numeric_limits<double>::max();
    for (double i = 0.0; i <= dMax; i++) {
        Position point1(obj1.getPosition().getX() + (obj1.getVelocity().getDx() * i / dMax),
                     obj1.getPosition().getY() + (obj1.getVelocity().getDy() * i / dMax));
        Position point2(obj2.getPosition().getX() + (obj2.getVelocity().getDx() * i / dMax),
